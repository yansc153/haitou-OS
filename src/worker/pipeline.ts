...
export class PipelineOrchestrator {
  ...
  async runDiscoveryCycle(teamId: string): Promise<number> {
    const CYCLE_TIMEOUT_MS = 180_000; // 3 minutes max for entire discovery cycle
    ...
    for (const platform of platforms) {
      // Timeout guard: stop if cycle exceeds 3 minutes
      if (Date.now() - cycleStart > CYCLE_TIMEOUT_MS) {
        console.warn(`[pipeline] Discovery cycle timeout (${CYCLE_TIMEOUT_MS}ms), stopping — processed so far, skipping remaining platforms`);
        break;
      }
